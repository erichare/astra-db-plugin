...
2. Call the `collection_card` tool (astra-widgets server) with `collection` (and `keyspace` when provided). On missing credentials, point the user to `/astra-db:setup` and stop; on `collection_not_found`, list the available names from the error and stop.
...
Offer `/astra-db:explore <name>` and `/astra-db:similar "<query>" --collection <name>` (or `--doc <id>` for collections without vectorize).
