...
1. Call the `database_overview` tool (astra-widgets server). If it reports missing credentials, tell the user to run `/astra-db:setup` or export `ASTRA_DB_APPLICATION_TOKEN` and `ASTRA_DB_API_ENDPOINT`, then stop.
2. Keep the tool's `structuredContent` — it is the data every rendering path below uses.

## Render

Follow the rendering ladder in the astra-widgets skill (`${CLAUDE_PLUGIN_ROOT}/skills/astra-widgets/SKILL.md`): inline widget if the harness offers a widget-rendering tool (use `templates/overview.html` as the blueprint); otherwise call the tool again with `emit: "html_file"` and open the returned path; always finish with a two-line markdown summary (totals, then the most notable collections).

## Follow-ups

Offer the obvious next steps by name: `/astra-db:collection <name>` for details, `/astra-db:explore <name>` to browse documents, `/astra-db:similar "<query>" --collection <name>` for vector search.
