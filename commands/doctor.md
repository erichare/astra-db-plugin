...
1. **Astra CLI installed** — run `astra --version`. Fix: run `/astra-db:setup` to install it.
...
6. **Database reachable** — if the CLI is authenticated, run `astra db list` and report the databases and their statuses. Fix for an empty list: create one via `/astra-db:setup`.
...
Output a table with columns Check / Result / Fix, then one sentence stating overall health. If every check passes, suggest a next step: ask for a collection or table to be created, or run `/astra-db:data-model-review` on the existing data layer.
