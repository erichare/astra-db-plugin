...
- **Tokens.** Access tokens live 1 hour. Refresh tokens rotate on every use and live 30 days from the last refresh, and at most 90 days from the original consent. With a replay store (below), each refresh token works exactly once.
...
**Replay protection.** By default the server keeps no state, so a refresh token that has already been used stays valid until it expires: a copied token keeps working. Connect a Redis REST store (Vercel KV / Upstash for Redis: `KV_REST_API_URL` and `KV_REST_API_TOKEN`, or `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN`) and each refresh token works once. Presenting a used one revokes that whole chain of rotations, so both the thief and the user must reconnect. If the store is unreachable, refreshes fail with a retryable 503 rather than skipping the check. Access tokens already issued stay valid for their remaining hour either way.

Without a store: to cut off every outstanding token, rotate the server secret. To cut off one user, revoke their Astra token in the Astra console; every hosted token carries it, so all of them stop working.
...
3. Optional but recommended: add a Redis store (*Storage → Upstash for Redis*, or any Redis REST endpoint) for [replay protection](#how-the-oauth-server-works). The integration sets the environment variables.
