...
Review the current project's Astra DB data model and Data API usage. Use the astra-data-modeler mode for the design assessment and, if data-access code exists, the astra-reviewer mode for usage review — run them in parallel when both apply.
...
When installed as a plugin, these files live under `.bob/skills/astra-toolkit/`.
