...
335 snippets in [examples/](examples/). Each file is `examples/<category>-<operation>[-<variant>].java`;
...
- `client-get-db-admin-from`: admin, db
- `client-no-options`
- `client-options`
- `client-token`
...
- `admin-create`: database, keyspace
- `admin-create-keyspace`: update
- `admin-drop`: database, keyspace
- `admin-find`: database, embedding-providers, reranking-providers
- `admin-find-database`: regions
- `admin-list`: databases, keyspaces
...
- `collections-find-distinct`: basic, nested, subset
...
- `collections-insert-one-id`: int, uuid
...
- `collections-replace-one`: escape, filter, id, nested, upsert, vector-2, vectorize-2
...
- `tables-drop-table`: exists, name, object
- `tables-drop-type`: exists
- `tables-filter`: all, eq, gt, gte, in, keys, lt, lte, ne, nin, values
- `tables-filter-all`: map
- `tables-filter-compound`: logic, range
- `tables-filter-in`: map
- `tables-filter-nin`: map
- `tables-find-distinct`: all, filter
...
- `tables-find-one`: combo, exclude, filter, include, lexical, similarity, sort, vector, vectorize
- `tables-find-one-lexical`: match, relevance
- `tables-get-table`: default, keyspace, typed
...
- `vectorize-`
- `vectorize-alter-table`: add, enable
- `vectorize-table`: (base), typed
